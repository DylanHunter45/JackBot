...
def load_user_data():
    ...
    try:
        ...
        logger.log_error(f"Error loading user data: {e}")
...
def save_user_data(data):
    ...
    try:
        ...
        logger.log_error(f"Error saving user data: {e}")
...
def should_claim_daily(user_data):
    ...
    try:
        ...
        logger.log_error(f"Error checking daily claim: {e}")
...
def increase_multiplier(user_data):
    ...
    try:
        ...
        logger.log_error(f"Error increasing multiplier: {e}")
...
class Coins(commands.Cog):
    ...
    async def opt_in(self, interaction: discord.Interaction):
        self.logger.log_info(f"Opt in called by {interaction.user}")
    ...
    async def check_balance(self, interaction: discord.Interaction):
        self.logger.log_info(f"Balance check called by {interaction.user}")
        user_id = str(interaction.user.id)
    ...
    async def claim_daily(self, interaction: discord.Interaction):
        self.logger.log_info(f"Daily claim called by {interaction.user}")
        user_id = str(interaction.user.id)
        if user_id not in self.user_data:
            await interaction.response.send_message(f'{interaction.user.mention} You are not opted in! Use /opt to start earning coins!')
        else:
            if should_claim_daily(self.user_data[user_id]):
                ...
                save_user_data(self.user_data)
                await interaction.response.send_message(f'{interaction.user.mention} You have claimed your daily coins! Your new balance is {self.user_data[user_id]["coins"]}!')
            else:
    ...
    async def multiplier(self, interaction: discord.Interaction):
        self.logger.log_info(f"Multiplier check called by {interaction.user}")
        user_id = str(interaction.user.id)
        if user_id not in self.user_data:
            await interaction.response.send_message(f'{interaction.user.mention} You are not opted in! Use /opt to start earning coins!')
        else:
